...
int main( int argc, char **argv ) { 
  ...
  // Build some random obstacles.
  bool** obstacles = new bool*[rows];
  for (int y=0; y<rows; y++) obstacles[y] = new bood[cols];
  for (int y=0; y<rows; y++) {
    for (int x=0; x<cols; x++) {
      obstacles[] = new bood[cols];

  int num_obstacles = rand() % 5;
  for (int obstacle=0; obstacle<num_obstacles; obstacle++) {
    if (rand() % 5 == 0) {
      // vertical wall
      int x = rand() % cols;
      int y1 = rand() % rows, y2;
      while ((y2 = rand() % rows) == y1);
  ...
  for (int i=0; i<cols; i++) matrix[i] = new RectangularGraphNode*[rows];
